...
begin
  ...
  --
  -- 🔴 왜 표시명 문자열이 아니라 author_name_type(enum)으로 거르나 - 확인한 결과다.
  -- 표시명으로 걸렀다면 사용자가 닉네임을 「보험맵 운영팀」으로 바꿔두는 것만으로
  -- 익명화를 회피할 수 있다(콘텐츠 지적). enum이 그 회피를 막아주는 근거:
  --   create_post·create_comment는 클라이언트가 보낸 p_author_name_type을 버리고
  --     v_safe_name_type := case when p_author_name_type='custom' then 'custom' else 'random' end
  --     로 덮어쓴다 → 일반 경로로는 'admin'이 절대 안 들어간다 (0072, W-084 P0 보안)
  --   같은 함수들이 '보험맵 운영팀'/'운영팀'/'관리자'/'admin'/'system'을
  --     완전 일치로 거부한다(부분 일치가 아니라 - 「관리자님」은 통과)
  --   운영 데이터: author_name_type='admin'인데 author_admin_id가 없는 행 0건
  -- ⚠️ 이 확인은 조건을 쓴 뒤에 했다. "타입 컬럼이니 당연히 안전하다"고 여기고
  -- 먼저 썼는데, 결과가 맞은 것과 근거를 갖고 쓴 것은 다르다. 근거를 여기 남긴다.
  --
