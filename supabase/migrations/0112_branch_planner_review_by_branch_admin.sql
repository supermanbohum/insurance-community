-- =========================================================
-- 0112_branch_planner_review_by_branch_admin.sql
-- 설계사의 지점 연결 승인은 **해당 지점 관리자**가 한다(오너 지시 2026-08-13).
--
-- ⚠️ 이 파일은 원래 `0111_`이었다. 0111 자리는 같은 날 **먼저 운영에 적용된**
-- `signup_ga_admin` 이메일 폴백(0111_signup_ga_admin_email_fallback.sql)이 가져가야 해서
-- 뒤로 밀었다. 이 저장소는 `supabase_migrations.schema_migrations` 기록이 0084에서
-- 끊겨 있고 0085~는 SQL 편집기 직접 실행이라, **파일 번호가 적용 순서의 유일한 기록**이다.
--
-- ---------------------------------------------------------
