...
--   사무실 사진    최소 3장 → 최소 5장
--   대표 홍보사진  존재 검사 유지 (상한 3장은 0099가 업로드 시점에 막는다)
--
...
--
-- 🔴 상한(대표 3장·사무실 10장)은 여기서 보지 않는다. 승인 시점에 상한을 걸면,
-- 이미 11장이 올라간 건을 관리자가 손댈 방법이 없어 영구 승인 불가 상태가 된다.
-- 상한은 업로드(add_branch_media)에서, 하한은 승인에서 본다.
--   대표 3장 상한 → 0099에서 적용 완료
--   사무실 10장 상한 → 아직 없음. 별도 마이그레이션 필요(CTO 확인 대기)
--
