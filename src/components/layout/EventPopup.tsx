...
type SnoozeChoice = 'none' | 'today' | 'week';

/** 팝업 인스턴스(=DB 행 id)별로 키를 분리한다 - 관리자가 새 이벤트로 내용을 바꾸면
 * id가 바뀌므로, 이전 이벤트를 닫았던 사람에게도 새 이벤트는 다시 뜬다.
 * "다시 보지 않기" 체크박스 UI는 제거했지만, 과거에 이미 그 선택을 저장한
 * 사용자의 localStorage 값(FOREVER)은 계속 존중한다(isCurrentlyHidden이 그대로 읽음). */
