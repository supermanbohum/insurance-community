...
- **이미 만든 마이그레이션 파일을 고쳐도 되는가**(2026-08-11 확정): **적용 여부로 갈린다.**
  ...
  **한 번도 실행된 적 없는 중간판이 이력에 남아**, 나중에 새 환경에 순서대로 적용할 때
  ...
  적는다.**
- **외부 이벤트 수신(웹훅 등)을 켜기 전에**(2026-08-11 확정): **그 이벤트가 실행할 코드
  경로를 끝까지 따라가 확인한다 — 화면에 닿는 지점까지.**

  🔴 「DB 수정 적용 → 그 다음 수신 켜기」라는 순서로만 적으면 작동하지 않는다. **적용된
  코드가 틀렸을 때는 순서를 지켜도 똑같이 당하기 때문**이다. 실제 사례: `0103`은 적용돼
  있었고 순서도 지켰는데, 익명화가 화면이 읽지 않는 컬럼을 바꾸고 있었다. 켜는 시점에
  "이 이벤트가 부르는 함수가 화면에 무엇을 남기는가"를 따라갔으면 그 자리에서 걸렸다.

  순서는 그 확인의 **결과**이지 규칙 자체가 아니다.

- **수신을 껐다 켜는 것은 되돌릴 수 있는 조치가 아니다**: 카카오 문서 기준 재전송은
  **전달 실패**(무응답·규격 위반 응답)에만 적용되고, **수신 설정이 꺼져 있던 기간의
  이벤트를 나중에 소급 발송한다는 규정은 없다.** 그 기간에 연결을 해제한 사용자는
  탈퇴 처리가 아예 실행되지 않고, 누가 해제했는지 기록도 남지 않아 **수동 복구도
  불가능하다.** 결함이 있을 때 "일단 수신을 끄자"는 안전한 임시 조치가 아니다.
  ⚠️ 같은 문서에 **연속 전송 실패 시 웹훅이 비활성화될 수 있다**고도 적혀 있다 —
  우리 라우트가 검증 실패에 400(규격 준수)을 주는 것은 실패로 세지 않지만, 500(우리 쪽
  장애)이 연속되면 웹훅 자체가 꺼질 수 있다.

- **위험을 비교할 때는 "몇 건이 실제로 걸리는가"를 함께 센다**: 결함 두 개를 같은 무게로
  놓으면 판단이 틀린다. 실제 사례로, 「익명화 미동작(대상 0건)」과 「탈퇴 처리 자체 미실행
  (대상 전원)」을 같이 놓고 "둘 다 위험"이라고 보고한 적이 있다 — 후자가 압도적으로 나쁘다.

- **화면을 잴 때 나를 속인 방법들**(2026-08-13, 하루에 세 세션이 각자 다른 자리에서 밟았다):

  | 재는 것 | 쓸 것 | 쓰면 안 되는 것 (그리고 왜) |
  |---|---|---|
  | 숨은 반응형 변형까지 포함한 존재 | `outerHTML` / `textContent` | `innerText` — `hidden sm:block`을 빼고 준다 |
  | 카운트업 애니메이션이 걸린 숫자 | 요소의 `textContent` | `innerText` — 애니메이션 중 숫자를 반영 안 한다 |
  | 줄이 어디서 접히는지 | `Range`로 글자별 `getClientRects()` | 문자열·노드 매칭 — `(선착순 <span>100</span>개 지점)`처럼 **노드가 쪼개지면 0건**이 나온다 |
  | 글자 폭 | **라이브 origin**에서 `Range` | 주입한 하네스 — CSS 청크가 없거나, `style.font` 단축이 next/font 패밀리를 날리거나, 폰트가 로드도 안 된 채 시스템 폰트로 재진다 |
  | 어떤 글자인지 | 코드포인트 추출 | 눈 — `·`(U+00B7 0.3261) / `•`(U+2022 0.3781) / `ㆍ`(U+318D 0.92)가 3배 차이다 |

  🔴 **공통점은 「오류가 안 나고 그럴듯한 값이 나온다」**는 것이다. 틀린 측정이 예외를
  던지면 아무도 안 속는다 — **그럴듯해서 속는다.**

  처방:
  - **다른 방법 둘이 같은 값을 내야 믿는다.** 같은 하네스로 두 번 재는 건 근거가 아니다.
    반대로 **지나치게 잘 맞으면**(서로 다른 폰트가 소수점 4자리까지 일치) 그게 오염 신호다.
  - **로컬 서버는 낡은 빌드를 서빙할 수 있다.** 재기 전에 **서빙 중인 마크업에 새 문자열이
    있는지부터** 확인한다. `position:sticky`가 `static`으로 나와 「sticky가 안 걸린다」고 볼
    뻔했는데 낡은 서버였다.
  - **측정값에 조건을 붙여 보고한다.** 「0건」이 아니라 「0건(375px·innerText)」이면 받는
    사람이 「PC에서 보셨나요」를 물을 수 있다.
  - 문구 교체는 **양방향**으로 센다 — 신규 N건뿐 아니라 **구문구 0건**까지.
